...
export function MyHeading() {
  ...
  function scrollMenu() {
    ...
    if (scrollY < Config.Menu.trigger) {
  ...
  useEffect(() => {
    if (!menuDaldo.loaded) {
      const fetch = async () => {
        ...
        const menu = await gway.getMenu(Config.Menu.reference);
        setMenuDaldo({
          ...menuDaldo,
          loaded: true,
          data: MenuWww.update(menu, Config.Menu.prefUri, window.location.pathname)
        });
      };
      fetch();
    }
